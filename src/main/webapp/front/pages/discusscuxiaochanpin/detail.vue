...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscusscuxiaochanpinCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscusscuxiaochanpinCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discusscuxiaochanpin');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','discusscuxiaochanpin');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('discusscuxiaochanpin', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscusscuxiaochanpin', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			// 添加评论
			async onCommentTap() {
				this.$utils.jump(`../discussdiscusscuxiaochanpin/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('discusscuxiaochanpin', this.detail);
				this.$utils.msg('审核成功');
