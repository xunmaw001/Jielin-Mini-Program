...
<script>
	export default {
		...
		async onShow(options) {
			...
			this.getStoreup();
			let cleanType = uni.getStorageSync('discusscuxiaochanpinCleanType')
			if(cleanType){
				uni.removeStorageSync('discusscuxiaochanpinCleanType')
				this.mescroll.num = 1
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','cuxiaochanpin');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				uni.setStorageSync('crossTable','cuxiaochanpin');
			...
			// 获取详情
			async init(){
				let res = await this.$api.info('cuxiaochanpin', this.id);
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discusscuxiaochanpin', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discusscuxiaochanpin/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
			...
			async onFinishTap() {
				...
				await this.$api.update('cuxiaochanpin', this.detail);
				this.$utils.msg('审核成功');
